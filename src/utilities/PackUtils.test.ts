...
describe('PackUtils', () => {
  ...
  describe('getOptimalEquipmentPack', () => {
    ...
    test('Fighter with 80gp gets chainmail (budget 40 = chainmail 40)', () => {
      const pack = getOptimalEquipmentPack(getClass('Fighter'), 80);
    ...
    test('Fighter with 120gp gets plate mail (budget 60 = plate 60)', () => {
    ...
    test('Fighter with 140gp gets plate mail, sword, shield and long bow', () => {
      // armour budget 70 ≥ plate 60 ✓; after plate+sword+shield+gear there is
      // enough gold left for a long bow.
      const pack = getOptimalEquipmentPack(getClass('Fighter'), 140);
      expect(hasItem(pack, 'plate_mail')).toBe(true);
      expect(hasItem(pack, 'sword')).toBe(true);
    ...
    test('Cleric gets warhammer (not sword) and holy symbol (silver in B/X)', () => {
      const pack = getOptimalEquipmentPack(getClass('Cleric'), 80);
      expect(hasItem(pack, 'sword')).toBe(false);
      expect(hasItem(pack, 'warhammer')).toBe(true);
    ...
    test('Magic-User gets no armour and uses staff', () => {
    ...
    test('Dwarf cannot use long bow or two-handed sword', () => {
      const pack = getOptimalEquipmentPack(getClass('Dwarf'), 140);
      expect(hasItem(pack, 'long_bow')).toBe(false);
