...
describe('getSpellTierGained', () => {
  it('Bug fix: MU L1→L2 gains an extra tier-0 slot — returns 0 (was: -1 / 0 meaning no pick)', () => {
    ...
    expect(getSpellTierGained(mu, 1, 2, DEFAULT_SPELL_SLOT_TABLES)).toBe(0);
  });

  it('Bug fix: MU L2→L3 newly unlocks tier-1 (L2 spells) → returns 1', () => {
    // L2 slots: [2,0,0,0,0,0] → L3 slots: [2,1,0,0,0,0]
    expect(getSpellTierGained(mu, 2, 3, DEFAULT_SPELL_SLOT_TABLES)).toBe(1);
  });

  it('MU L3→L4 gains an extra tier-1 slot → returns 1', () => {
    // L3 slots: [2,1,0,0,0,0] → L4 slots: [2,2,0,0,0,0]
    expect(getSpellTierGained(mu, 3, 4, DEFAULT_SPELL_SLOT_TABLES)).toBe(1);
  });

  it('MU L4→L5 newly unlocks tier-2 (L3 spells) → returns 2', () => {
    // L4 slots: [2,2,0,0,0,0] → L5 slots: [2,2,1,0,0,0]
    expect(getSpellTierGained(mu, 4, 5, DEFAULT_SPELL_SLOT_TABLES)).toBe(2);
  });

  it('Fighter L1→L2 returns -1 (no spell slots ever)', () => {
    expect(getSpellTierGained(fighter, 1, 2, DEFAULT_SPELL_SLOT_TABLES)).toBe(-1);
  });
  ...
  it('returns the HIGHEST gained tier when multiple tiers increase in one level-up', () => {
    ...
    expect(getSpellTierGained(mu, 1, 3, DEFAULT_SPELL_SLOT_TABLES)).toBe(1);
  });
});

describe('getAvailableSpellsAtTier', () => {
  it('returns L1 MU spells (tier 0) when no spells are known', () => {
    const available = getAvailableSpellsAtTier(magicUserSpellsByLevel, 0, []);
  ...
  it('returns L2 MU spells (tier 1) when no spells are known', () => {
    const available = getAvailableSpellsAtTier(magicUserSpellsByLevel, 1, []);
  ...
  it('excludes already-known spells from the results', () => {
    const available = getAvailableSpellsAtTier(magicUserSpellsByLevel, 0, ['sleep', 'magic-missile']);
  ...
  it('returns empty array for empty byLevel (non-caster)', () => {
    expect(getAvailableSpellsAtTier([], 0, [])).toEqual([]);
  });
  ...
  it('falls back to the last available tier when spellTier is out of range', () => {
    ...
    const available = getAvailableSpellsAtTier(magicUserSpellsByLevel, 10, []);
...
describe('getSpellTiersGained', () => {
  it('Bug fix: MU L6→L7 gains tier 0 (L1) AND tier 3 (L4) — returns [0, 3]', () => {
    ...
    expect(getSpellTiersGained(mu, 6, 7, DEFAULT_SPELL_SLOT_TABLES)).toEqual([0, 3]);
  });

  it('MU L1→L2 gains only tier 0 → returns [0]', () => {
    expect(getSpellTiersGained(mu, 1, 2, DEFAULT_SPELL_SLOT_TABLES)).toEqual([0]);
  });

  it('MU L2→L3 gains only tier 1 → returns [1]', () => {
    expect(getSpellTiersGained(mu, 2, 3, DEFAULT_SPELL_SLOT_TABLES)).toEqual([1]);
  });

  it('Fighter L1→L2 has no spell slots → returns []', () => {
    expect(getSpellTiersGained(fighter, 1, 2, DEFAULT_SPELL_SLOT_TABLES)).toEqual([]);
  });

  it('MU L3→L4 gains only tier 1 → returns [1]', () => {
    expect(getSpellTiersGained(mu, 3, 4, DEFAULT_SPELL_SLOT_TABLES)).toEqual([1]);
  });
});
...
describe('Bug regression: L2→L3 spell selection shows L2 spells', () => {
  it('getSpellTierGained returns 1 (tier-1 = L2 spells) for MU L2→L3', () => {
    const tier = getSpellTierGained(mu, 2, 3, DEFAULT_SPELL_SLOT_TABLES);
    expect(tier).toBe(1);
  });

  it('getAvailableSpellsAtTier with tier 1 returns L2 spells (not L1 spells)', () => {
    const available = getAvailableSpellsAtTier(magicUserSpellsByLevel, 1, ['Sleep']); // 'Sleep' is L1, irrelevant
